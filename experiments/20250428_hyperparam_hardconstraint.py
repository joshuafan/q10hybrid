"""
Hyperparameter tuning for hybrid 1-layer KAN model: 
grid=30, grid_margin=1.0, primarily tuning regularization loss weights.

synthetic Rb = np.log(ds["sw_pot"] - ds["sw_pot"].min() + 0.1) + np.log(ds["dsw_pot"] - ds["dsw_pot"].min() + 0.1)
...
class Objective(object):
    ...
    def __call__(self, trial: optuna.trial.Trial) -> float:
        ...
        lambda_kan_l1 = trial.suggest_float('lambda_kan_l1', 1e-3, 1e-2, log=True)
    ...
    def add_project_specific_args(parent_parser: ArgumentParser) -> ArgumentParser:
        ...
        parser.add_argument(
            '--log_dir', default='./logs/20250430_synth3_hardconstraint_grid30/', type=str)
        return parser
